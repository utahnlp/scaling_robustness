...
def transform_to_t5_targets(labels):
    targets = []
    # sentiment SST2, 0-> negative, 1 -> positive
    for lb in labels:
        if lb == 1:
...
def transform_from_t5_targets(targets):
    labels = []
    for tar in targets:
...
def parse_args():
    ...
    parser.add_argument(
        "--do_train",
        action="store_true",
        help="If passed, do training.",
    )
    parser.add_argument(
        "--do_eval",
        action="store_true",
        help="If passed, do evaluation.",
    )
...
def main():
    ...
    # NOTE: Only works for SST2 for now
    def preprocess_function(examples):
        ...
        if "label" in examples:
            targets = transform_to_t5_targets(examples['label'])
    ...
    def postprocess_text(preds, labels):
        ...
        return transform_from_t5_targets(preds), transform_from_t5_targets(labels)
    ...
    if args.do_train:
        ...
        for epoch in range(starting_epoch, args.num_train_epochs):
            ...
            model.eval()
            logger.info("Going to do evaluation for this epoch")
    ...
    if args.do_eval:
        ...
        for step, batch in enumerate(eval_dataloader):
            with torch.no_grad():
                ...
                # print(outputs_dict["scores"])
                # print(outputs_dict["scores"][0].shape)
                # print(outputs_dict["scores"][1].shape)
                # print(outputs_dict["scores"][0].topk(5))
                # print(outputs_dict["scores"][1].topk(5))
                # generated_tokens = outputs_dict["sequences"]
                ...
                progress_bar.update(1)
        eval_metric = metric.compute()
        logger.info(f"Final Evaluation score: {eval_metric}")
