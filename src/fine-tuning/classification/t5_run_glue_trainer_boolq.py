...
task_to_keys = {
    ...
    "lasha-nlp/CONDAQA": ('sentence2', 'sentence1'),
    "drop": ('question', 'passage'),
    "quoref": ('question', 'context'),
    "ropes": ('question', 'context'),
...
t5_task_prefix = {
    ...
    "lasha-nlp/CONDAQA": ('condaqa question: ', 'passage: '),
    "drop": ('drop question: ', 'passage: '),
    "quoref": ('quoref question: ', 'passage: '),
    "ropes": ('ropes question: ', 'passage: '),
...
def extract_prediction_prob(scores, task, tokenizer, scheme='first', return_prob=True):
    ...
    for lb_int, lb_str in INT_LABEL_TO_STR[task.lower()].items():
    ...
    if scheme == 'first':
        ...
        for i in range(len(label_to_logits)):
            label_to_logits_dict[INT_LABEL_TO_STR[task.lower()][i]] = label_to_logits[i]
...
def prediction_loop(model, trainer, tokenizer, predict_dataset, task):
    ...
    for step, inputs in enumerate(dataloader):
        ...
        #    returned_outputs.append(gen_toks)
        ...
        #    for j in range(len(generated_dict.scores)):
        #        seq_scores[j] = generated_dict.scores[j][i]
        #    logit_scores = extract_prediction_prob(seq_scores, "qqp", tokenizer)
        #    inps = inputs['input_ids'][0].tolist()
        #    #print(inps)
        #    #print(gen_toks)
        #    #print(logit_scores)
...
def compute_accuracy_conda_qa(preds, labels):

    correct = 0.0
    total = 0.0

    for i in range(len(preds)):
        if preds[i].lower().strip() == labels[i].lower().strip():
            correct +=1
        total +=1
...
def extract_label_for_drop(dataset):

    labels = []
    for ex in dataset:
        labels.append(ex['answers_spans']['spans'][0])

    new_dataset = dataset.add_column("label", labels)

    return new_dataset

def extract_label_for_quoref(dataset):

    labels = []
    for ex in dataset:
        labels.append(ex['answers']['text'][0])

    new_dataset = dataset.add_column("label", labels)

    return new_dataset

def extract_label_and_contexts_for_ropes(dataset):

    labels = []
    contexts = []
    for ex in dataset:
        labels.append(ex['answers']['text'][0])
        contexts.append(ex['situation'] + ' ' + ex['background'])

    new_dataset = dataset.add_column("label", labels)
    new_dataset_2 = new_dataset.add_column("context", contexts)
...
def main():
    ...
    if 'drop' in data_args.task_name:
        new_raw_datasets = type(raw_datasets)()
        new_raw_datasets['train'] = extract_label_for_drop(raw_datasets['train'])
        new_raw_datasets['validation'] = extract_label_for_drop(raw_datasets['validation'])
        raw_datasets = new_raw_datasets
    elif 'quoref' in data_args.task_name:
        new_raw_datasets = type(raw_datasets)()
        new_raw_datasets['train'] = extract_label_for_quoref(raw_datasets['train'])
        new_raw_datasets['validation'] = extract_label_for_quoref(raw_datasets['validation'])
        raw_datasets = new_raw_datasets
    elif 'ropes' in data_args.task_name:
        new_raw_datasets = type(raw_datasets)()
        new_raw_datasets['train'] = extract_label_and_contexts_for_ropes(raw_datasets['train'])
        new_raw_datasets['validation'] = extract_label_and_contexts_for_ropes(raw_datasets['validation'])
        raw_datasets = new_raw_datasets
    elif 'boolq' in data_args.task_name:
        new_raw_datasets = type(raw_datasets)()
    ...
    #if 'conda' in data_args.task_name.lower():
    ...
    def compute_metrics(eval_preds):
        ...
        result = compute_accuracy_conda_qa(decoded_preds, decoded_labels)
