...
class ModelArguments:
    ...
    only_print_num_params: bool = field(
        default=False,
        metadata={"help": "Only print number of parameters"}
    )
...
def main():
    ...
    if data_args.task_name is not None:
        ...
        if data_args.task_name in ['snli']:
            raw_datasets = load_dataset(data_args.task_name)
            label_set = set([ex['label'] for ex in raw_datasets['train'] ])
            # print(label_set, len(raw_datasets['train']))
            train_raw_datasets = raw_datasets['train'].filter(lambda example: example['label'] != -1)
            raw_datasets['train'] = train_raw_datasets
            val_raw_datasets = raw_datasets['validation'].filter(lambda example: example['label'] != -1)
            raw_datasets['validation'] = val_raw_datasets
            label_set = set([ex['label'] for ex in raw_datasets['train'] ])
            # print(label_set, len(raw_datasets['train'] ))
            # sss
        else:
        ...
        if 'boolq' in data_args.dataset_name:
    ...
    if training_args.do_predict and data_args.test_file is not None:
        ...
        #prediction_dataset = load_dataset("csv", data_files={'predict':data_args.test_file})
        input_readers = []
    ...
    print('=='*24)
    if model_args.only_print_num_params:
        print('Exiting the program')
        sys.exit(0)

    if 'gpt2' in model_args.model_name_or_path:
        tokenizer.pad_token = tokenizer.eos_token # required for GPT models (https://github.com/huggingface/transformers/issues/3859)
    ...
    if data_args.task_name is not None:
        ...
        if data_args.dataset_name is not None and data_args.dataset_name in task_to_keys:
            sentence1_key, sentence2_key = task_to_keys[data_args.dataset_name]
        else:
            # Again, we try to have some nice defaults but don't hesitate to tweak to your use case.
            non_label_column_names = [name for name in raw_datasets["train"].column_names if name != "label"]
            print('Columns in raw_datasets["train"] are ', raw_datasets["train"].column_names)
    ...
    print('Column keys are: sentence1_key:{}, sentence2_key:{}'.format(sentence1_key, sentence2_key))
    # Padding strategy
    ...
    def preprocess_function(examples):
        ...
        #     filtered_examples = [ex for ex in examples if ex['label'] != -1]
        #     skipped += len(examples) - len(filtered_examples)
        ...
        if label_to_id is not None and "label" in examples and not training_args.do_predict:
    ...
    if training_args.do_predict:
        ...
        # Loop to handle MNLI double evaluation (matched, mis-matched)
        #print('Number of data points ', len(predict_dataset['predict']))
        #tasks = [data_args.task_name]*len(predict_datasets_list)
        #if data_args.task_name == "mnli" and data_args.test_file is not None:
        #    tasks.append("mnli-mm")
        #    predict_datasets.append(raw_datasets["test_mismatched"])
        #for predict_dataset, task in zip(predict_datasets_list, tasks):
        ...
        for dataset_itr in range(len(input_readers)):
            ...
            prediction_probs = torch.nn.functional.softmax(torch.Tensor(predictions), dim=-1)
            #print(prediction_probs.shape) # has shape (num_examples, num_classes)
            ...
            if trainer.is_world_process_zero():
                if data_args.prediction_format.lower() == 'orig_default':
                    ...
                    index_in_predictions = None
                    for itr in range(len(label_list)):
                        if label_list[itr].lower() == LABEL_TO_PRINT:
                            index_in_predictions =  itr
                            break
                    print(index_in_predictions)
                    print(label_list)
                    with open(output_predict_file, "w") as writer:
                        logger.info(f"***** Predict results {task} *****")
                        prediction_probs_list = prediction_probs.tolist()
                        for index, item in enumerate(prediction_probs_list):
                            pred_prob = prediction_probs_list[index][index_in_predictions]
                            writer.write(str(pred_prob) + '\n')
                elif data_args.prediction_format.lower() == 'default':
                    name = data_args.task_name if data_args.task_name is not None else data_args.dataset_name
                    if name not in INT_LABEL_TO_STR:
                        label_id_to_str = {v:k for k,v in model.config.label2id.items()}
                        #print(model.config.label2id)
                    else:
                        label_id_to_str = INT_LABEL_TO_STR[name]
                    #print(type(prediction_dataset))
                    ...
                    assert len(prediction_dataset) == prediction_probs.shape[0]
                    output_data = []
                    for ex_idx in range(len(prediction_dataset)):
                        ...
                        for k, v in prediction_dataset[ex_idx].items():
                            ex[k] = v
                        predicted_label = label_id_to_str[predictions[ex_idx]]
                        probs = prediction_probs[ex_idx].tolist()
                        probs_dict = {}
                        for x in range(len(probs)):
                            probs_dict[label_id_to_str[x]] = probs[x]
                        ex['probs'] = probs_dict
                    ...
                    #output_predict_file = os.path.join(training_args.output_dir, data_args.test_file[dataset_itr].split('/')[-1] + ".preds.jsonl")
