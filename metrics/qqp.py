...
from scorer import AccuracyScorer, F1Scorer, AucQQP
from metrics_utils import extract_model_details, write_list_of_dicts_as_csv
from copy import deepcopy
import pickle

FILE_NAME_TO_METRIC_DETAILS = {
        'robustness/twitter_ppdb/twitter_url_corpus_test.csv': {'evaluation': 'twitter ppdb', 'gold_key': 'label', 'type': None, 'subset': None, 'metric': 'accuracy'},

        'robustness/evaluation_sets/qqp/paws_qqp_dev_test.csv': {'evaluation': 'paws-qqp', 'gold_key': 'label', 'type': None, 'subset': None, 'metric': 'qqp_auc,accuracy'},

        'robustness/evaluation_sets/qqp/qqp_validation.csv': {'evaluation': 'qqp', 'gold_key': 'label', 'type': None, 'subset': None, 'metric': 'accuracy,f1', 'pred_label_map':{'not_duplicate':0, 'duplicate': 1}, 'gold_label_map':{'not_duplicate': 0, 'duplicate': 1}},
        }
...
if __name__=="__main__":
    ...
    for eval_set_path, eval_set_details in FILE_NAME_TO_METRIC_DETAILS.items():
        ...
        if 'gold_label_map' not in metric_details:
            gold_label_map = None
        else:
            gold_label_map = metric_details['gold_label_map']
        ...
        for metric_name in metric_details['metric'].split(','):
            if metric_name.strip() == '':
                continue
            metric = METRIC_TO_SCORER[metric_name](filename=model_dir + '/' + f, gold_key=gold_key, pred_key = 'predicted', pred_label_map=pred_label_map, gold_label_map=gold_label_map)
            metric.calculate()
